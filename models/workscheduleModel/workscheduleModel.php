<?php
    ...
    public static function get($wsid ,$date , $staffname , $staffpname , $stationname)
    {
        require("connection_connect.php");
        $sql="SELECT ws.date as d , sc.cname as cname , spc.name as n , station.s_name as  s_name
                FROM WorkSchedule as ws
                INNER JOIN StaffCheckpoint as sc ON sc.staff_c_id = ws.staff_c_id
                INNER JOIN StaffPositionCheckpoint as spc ON spc.staff_pc_id = ws.staff_pc_id
                INNER JOIN station ON station.station_id = ws.station_id
        ...
        $stationname = $my_row['s_name'];
        require("connection_close.php");
        return new wsModel($wsid , $date , $staffid , $staffname , $staffpid , $staffpname , $stationid , $stationname);
    }

    public static function update($wsid, $date , $staffname , $staffpname , $stationname)
    {
        require("connection_connect.php");
        $sql="UPDATE WorkSchedule SET date ='$date' , staff_c_id = '$staffname' , staff_pc_id = '$staffpname' , station_id = '$stationname'
        WHERE ws_id= '$wsid'";
        $result=$conn->query($sql);
        require("connection_close.php");
    ...
    public static function delete($wsid)
    {
         require("connection_connect.php");
         $sql="DELETE FROM WorkSchedule WHERE ws_id='$wsid'";
         $result=$conn->query($sql);
         require("connection_close.php");
